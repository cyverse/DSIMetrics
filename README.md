...
* **initialUploads** - Single run scripts that will crawl over locally downloaded CSV files for Zoom attendance and Qualtrics registrations. They will extract data from the CSVs and input into Posgresql database. ❗❗❗USE THESE SCRIPTS ❗❗❗
...
* **productionScripts** - ❗❗❗THESE SCRIPTS ARE NOT CURRENTLY OPERATIONAL ❗❗❗These scripts can be automatically run by crontab on Linux. These scripts are responsible for the main automation of the system including fetching qualities and zoom data and listening from new series to be entered through Budibase
